...
const multipartMiddleware = (next) => (req, res) => {
  ...
  const parsePart = (buffer) => {
    ...
    const contentEndIndex = buffer.indexOf(
      CRLF,
      contentStartIndex + CRLFCRLF.length,
    );
    if (contentEndIndex === -1) {
      throw new Error("bad multipart: no content in part");
    }
    part.content = buffer.slice(
      contentStartIndex + CRLFCRLF.length,
      contentEndIndex,
    );
    ...
    do {
      ...
      if (header.toLowerCase().startsWith("content-disposition")) {
        ...
        part.filename = /filename="(?<filename>[^"]*)"/.exec(
          header,
        )?.groups?.filename;
      }
  ...
  req.on("end", () => {
    try {
      ...
      do {
        const nextPartIndex = buffer.indexOf(
          startBoundary,
        ...
        entries() {
          ...
          return Iterator.from({
            next() {
              ...
              const nextPartIndex = buffer.indexOf(
                startBoundary,
                lazyIndex + startBoundary.length,
              );
              if (nextPartIndex === -1) {
                // last part
...
const wsHandler = (req, socket, head) => {
  ...
  const digest = crypto.hash(
    "sha1",
    wsKey.trim() + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11",
    "base64",
  );
...
class Router {
  constructor(...middlewares) {
    ...
    this.routes = new Map();
    this.notFoundHandler = [...this.middlewares].reduce(
      (prev, curr) => curr(prev),
      (req, res) =>
        this.constructor.json(req, res, {
          code: 404,
          body: { error: "not found" },
        }),
    );
    this.internalErrorHandler = [...this.middlewares].reduce(
      (prev, curr) => curr(prev),
      (req, res) =>
        this.constructor.json(req, res, {
          code: 500,
  ...
  register(method, path, handler, ...middlewares) {
    ...
    if (typeof path === "string" && path.match(this.variablePattern)) {
      const replaced = path.replaceAll(
        ...
        this.variablePatternReplacement,
      );
    ...
    const wrapped = [...middlewares.toReversed(), ...this.middlewares].reduce(
      (prev, curr) => curr(prev),
      handler,
    );
  ...
  handle(req, res) {
    ...
    const route = [...this.routes.get(req.method).entries()].find(([k, _]) =>
      k instanceof RegExp ? k.test(req.url) : k === req.parsedUrl.pathname,
    );
  ...
  static json(
    req,
    res,
    { code = 200, contentType = "application/json", data },
  ) {
    res.statusCode = code;
...
router.register("POST", "/api/test/tls", (req, res) => {
