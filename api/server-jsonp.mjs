...
router.register('GET', '/api/jsonp/getNoteById', (req, res) => {
    // req.params => regexp match => groups.noteId
    const noteId = Number.parseInt(req.parsedUrl.searchParams.get('noteId'), 10);
    const callback = req.parsedUrl.searchParams.get('callback') || 'handleNote';
    res.statusCode = 200;
    res.setHeader('Content-Type', 'application/javascript');

    if (!noteId) {
        res.end(`
            ${callback}(null, ${JSON.stringify({error: 'bad id'})});
        `);
        return
    }
    ...
    if (!note) {
        res.end(`
            ${callback}(null, ${JSON.stringify({error: 'not found'})});
        `);
        return
    }
